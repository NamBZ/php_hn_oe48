        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="section-tittle text-center mb-55">
                    <h2>{{ __('Search results for') }} `{{ request()->query('query') }}`</h2>
                </div>
            </div>
            ...
            @if ($list_products->isNotEmpty())
                ...
                @endforeach
                {{ $list_products->appends(['query' => request()->query('query')])->links() }}
            @else
                <div class="alert alert-primary" role="alert">{{ __('No matching products found for your selection') }}</div>
            @endif
